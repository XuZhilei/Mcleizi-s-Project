...
timepoint = 4067049273	  #randomly choose a match for test
...
def catch():
    ...
    while True:
        r = requests.get('http://api.steampowered.com/IDOTA2Match_570/GetLiveLeagueGames/v1?key=ACEB542E8E93033CACD987BD5DA301E6&league_id=9870')
        if json.loads(r.text) != {}:
            break
        time.sleep(3)
        print('Getting API data...')
        
    with open ('maineventlist{}.json'.format(time.strftime('%Y-%m-%d',time.localtime(time.time()))), 'r') as f:
        matchlist = json.loads(f.read())
    ...
    if r == []: #if no living match:
        ...
        for i in r: #traversing living match
            #if living match not in matchlist then add it in
            if i['radiant_team']['team_id'] < i['dire_team']['team_id']:
                tempid = [i['radiant_team']['team_id'], i['dire_team']['team_id']]
            else:
                tempid = [i['dire_team']['team_id'], i['radiant_team']['team_id']]
            if tempid not in [[k[0], k[1]] for k in matchlist]:
                matchlist.append([tempid[0], tempid[1], 6655, 1, 0, []])
                break
    ...
    with open ('maineventlist{}.json'.format(time.strftime('%Y-%m-%d',time.localtime(time.time()))), 'w') as f:
        f.write(json.dumps(matchlist))
    return 0

def writeformat(canshu):#0为难民，1为皇家，2为积分表
    with open ('maineventlist{}.json'.format(time.strftime('%Y-%m-%d',time.localtime(time.time()))), 'r') as f:
        matchlist = json.loads(f.read())
        #team1/team2/series/status(0=not yet, 1=live, 2=finished)/score(0, 1, 2, 10, 11, 20)/[matchid1, matchid2]
    ...
    for i in range(len(matchlist)):
        if i == 0:
            royal_text += '[tr]\n[td15]{series}\n[td25]{team1}[/td]\n[td15][align=center]{s1} - {s2}[/align][/td]\n[td25]{team2}[/td]\n[td20][/td][/tr]'.format(series=serieslist[i], team1=royal_team_name(matchlist[i][0]), team2=royal_team_name(matchlist[i][1]), s1=(matchlist[i][4] // 10), s2=(matchlist[i][4] % 10))
        else:
            royal_text += '[tr]\n[td15]{series}\n[td25]{team1}[/td]\n[td15][align=center]{s1} - {s2}[/align][/td]\n[td25]{team2}[/td]\n[td20][/td][/tr]'.format(series=serieslist[i], team1=royal_team_name(matchlist[i][0]), team2=royal_team_name(matchlist[i][1]), s1=(matchlist[i][4] // 10), s2=(matchlist[i][4] % 10))
